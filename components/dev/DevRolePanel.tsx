    ...
    <>
      ...
      >
        ...
        <View className="flex-row items-center">
          <Text className="font-mont-semibold text-sm text-text-primary capitalize">
            {effectiveRole}
          </Text>
          {devOverride && (
            <Text className="font-mont text-[10px] text-primary ml-1.5">(dev)</Text>
          )}
        </View>
      </Pressable>
      ...
      {visible && (
        <View
          ...
          style={{ backgroundColor: "rgba(26,75,95,0.10)", borderWidth: 1, borderColor: "rgba(26,75,95,0.2)" }}
        >
          <Text className="font-mont text-[10px] text-primary mb-2">
            Dev Role Override
          </Text>
          <View className="flex-row flex-wrap gap-1.5">
            <Pressable
              ...
              style={{ backgroundColor: !devOverride ? "#1A4B5F" : "rgba(26,75,95,0.10)" }}
            >
              <Text
                className="font-mont-medium text-xs"
                style={{ color: !devOverride ? "#FFFFFF" : "#5F6E63" }}
              >
                Real
              </Text>
            </Pressable>
            {devRoles.map((role) => (
              <Pressable
                ...
                className="px-3 py-1.5 rounded-lg"
                style={{ backgroundColor: devOverride === role ? "#1A4B5F" : "rgba(26,75,95,0.10)" }}
              >
                <Text
                  className="font-mont-medium text-xs capitalize"
                  style={{ color: devOverride === role ? "#FFFFFF" : "#5F6E63" }}
                >
                  {role}
                </Text>
              </Pressable>
