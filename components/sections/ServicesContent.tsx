...
export function ServicesContent({ showBack = false }: ServicesContentProps) {
      ...
      <View className="overflow-hidden rounded-card bg-card">
        <View className="relative">
          ...
          {item.images && item.images.length > 1 && (
            <View className="absolute top-2 right-2 flex-row items-center bg-background/60 rounded-full px-1.5 py-0.5">
              <Ionicons name="images-outline" size={10} color="#0D1A12" />
              <Text className="font-mont text-[10px] text-text-primary ml-0.5">
                {item.images.length}
              </Text>
            </View>
          )}
  ...
  return (
    <ScreenContainer>
      ...
      <View className="mx-4 h-px" style={{ backgroundColor: "rgba(26,75,95,0.06)" }} />
